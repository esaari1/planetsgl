...
Matrix4x4 M3DTriangleMesh::getMatrix() const
{
    return matrix;
}

void M3DTriangleMesh::setMatrix(const Matrix4x4& m)
{
    matrix = m;
}

Point3D M3DTriangleMesh::getVertex(uint16 n) const
{
    return points[n];
}
...
void M3DTriangleMesh::addVertex(const Point3D& p)
{
    points.insert(points.end(), p);
}

Point3D M3DTriangleMesh::getTexCoord(uint16 n) const
{
    return texCoords[n];
}
...
void M3DTriangleMesh::addTexCoord(const Point3D& p)
{
...
GLuint M3DScene::createDisplayList() {
   GLuint drawId = glGenLists(1);
   glNewList(drawId, GL_COMPILE);
   
   glBegin(GL_TRIANGLES);

   for(unsigned int modelIdx = 0; modelIdx < models.size(); modelIdx++) {
      M3DModel* model = models[modelIdx];
      uint16 face[3];

      for(uint32 triIdx = 0; triIdx < model->getTriMeshCount(); triIdx++) {
         M3DTriangleMesh* triangle = model->getTriMesh(triIdx);

         for(uint16 f = 0; f < triangle->getFaceCount(); f++) {
            triangle->getFace(f, face[0], face[1], face[2]);
         }
      }
   }
